/**
 * Shared query helpers for /manager routes.
 * Follows the same pattern as /supply/_components/query.ts & /kitchen/_components/query.ts
 */
...
export type ParsedManagerListQuery = {
    page: number;
    limit: number;
    sortOrder: SortOrder;
    search?: string;
...
export function parseManagerListQuery(
    searchParams: RawSearchParams,
    defaults?: Partial<ParsedManagerListQuery>,
): ParsedManagerListQuery {
    ...
    const warehouseIdRaw = readValue(searchParams.warehouseId);
    const warehouseId = warehouseIdRaw ? toPositiveInt(warehouseIdRaw, 0) || undefined : undefined;
    
    const productIdRaw = readValue(searchParams.productId);
    const productId = productIdRaw ? toPositiveInt(productIdRaw, 0) || undefined : undefined;
    ...
    return {
        ...
        warehouseId: warehouseId ?? defaults?.warehouseId,
