...
import { Product, ProductType, getProductBaseUnitDisplay } from "@/types/product";
...
export default function RecipeFormModal({ open, onClose, recipeId }: Props) {
    ...
    const { createRecipe, updateRecipe, productionRecipeDetail } = useProduction();
    ...
    const form = useForm<CreateRecipeFormValues>({
        resolver: zodResolver(CreateRecipeFormSchema) as Resolver<CreateRecipeFormValues>,
        defaultValues: {
            outputProductId: 0,
            standardOutput: 1,
    ...
    return (
        ...
        >
            <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col">
                <DialogHeader>
                    <DialogTitle>{isEdit ? "Sửa công thức (BOM)" : "Tạo công thức (BOM)"}</DialogTitle>
                    <DialogDescription>
                        Định mức gửi API là trên 1 đơn vị thành phẩm (quy đổi từ tổng nhập cho sản lượng chuẩn).
                    </DialogDescription>
                </DialogHeader>
                            ...
                            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel>
                                                Thành phẩm đầu ra <span className="text-red-500">*</span>
                                            </FormLabel>
                                            <Select
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <p className="text-[11px] text-slate-500">
                                                ...
                                                <strong>tổng cho {field.value || "—"}</strong> đơn vị thành phẩm.
                                            </p>
                                        </FormItem>
                                    )}
                                />
                                ...
                                <div className="space-y-3">
                                    ...
                                    {fields.map((f, index) => {
                                        ...
                                        const unitFromDetail = detail ? getProductBaseUnitDisplay(detail) : "—";
                                            ...
                                            >
                                                ...
                                                <FormField
                                                    ...
                                                    render={({ field: rowField }) => (
                                                        <FormItem>
                                                            <FormLabel className="text-xs">Nguyên liệu</FormLabel>
                                                            <Select
                                                                value={rowField.value ? String(rowField.value) : ""}
                                                                onValueChange={(v) => rowField.onChange(Number(v))}
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    ...
                                                    />
                                                    <FormItem>
                                                        <FormLabel className="text-xs">Đơn vị (theo NL)</FormLabel>
                                                        <Input
                                                            readOnly
                                                            value={unitLabel}
