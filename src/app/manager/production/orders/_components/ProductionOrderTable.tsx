...
export default function ProductionOrderTable({ items, isLoading, rowStart }: Props) {
    if (!isLoading && items.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-sm font-medium text-slate-500">Không có lệnh sản xuất</p>
        ...
        <Table>
            <TableHeader>
                <TableRow className="bg-slate-50/80 hover:bg-slate-50/80">
                    ...
                    <TableHead className="text-xs font-semibold text-slate-500">Mã lệnh</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-500">Công thức / TP</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-500 w-[100px]">Kế hoạch</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-500 w-[100px]">Thực tế</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-500 w-[120px]">Bếp</TableHead>
                    <TableHead className="text-xs font-semibold text-slate-500 w-[110px]">Trạng thái</TableHead>
                ...
                {isLoading ? (
                    ...
                    items.map((o, index) => (
                        <TableRow key={o.id} className="hover:bg-slate-50/50">
                            ...
                            <TableCell>
                                <p className="text-sm text-slate-900">{o.recipeName || o.productName}</p>
                            ...
                            <TableCell>
                                <Badge className={`border text-[10px] ${statusBadgeClass(o.status)}`}>
