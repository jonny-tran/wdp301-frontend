...
export default function BaseUnitCreateModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await createBaseUnit.mutateAsync(formData); // Gọi endpoint POST /base-units
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full rounded-full border border-slate-200 bg-slate-50/50 px-8 py-5 text-sm font-bold outline-none focus:border-blue-500 shadow-inner"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-4">
                ...
                <textarea
                  ...
                  className="w-full rounded-[2rem] border border-slate-200 bg-slate-50/50 px-8 py-5 text-sm font-bold outline-none focus:border-blue-500 shadow-inner"
                />
              </div>
            </div>
          </div>
          <div className="md:col-span-5 flex flex-col justify-between p-10 bg-slate-50/50 border-l border-slate-100 text-center">
            ...
            <div className="space-y-4">
              <button
                ...
                disabled={createBaseUnit.isPending}
