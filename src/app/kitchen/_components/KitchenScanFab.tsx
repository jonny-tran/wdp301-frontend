...
export default function KitchenScanFab() {
    ...
    const handleLookup = () => {
        const n = parseInt(rawId.replace(/\D/g, ""), 10);
        ...
        <>
            ...
            >
                <DialogContent className="max-w-md border-2 border-zinc-900 bg-white text-zinc-950">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-black">Tra cứu lô (QR / ID)</DialogTitle>
                        <DialogDescription className="text-base text-zinc-600">
                            Nhập ID lô từ nhãn hoặc sau khi quét (tích hợp máy quét có thể điền ô dưới).
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    {lookupId > 0 && q.isLoading && (
                        <p className="text-center text-sm font-medium text-zinc-500">Đang tải…</p>
                    )}
                    {lookupId > 0 && q.isError && (
                    ...
                    {q.data && (
                        <div className="space-y-2 rounded-xl border-2 border-emerald-200 bg-emerald-50/50 p-4 text-sm">
                            <p className="text-lg font-black text-zinc-900">{q.data.productName}</p>
                            <p className="font-mono font-bold text-emerald-800">Mã lô: {q.data.batchCode}</p>
                            <p>
                                HSD:{" "}
                            ...
                            <p>
                                Tồn hiện tại:{" "}
                                <span className="font-black tabular-nums text-zinc-900">{q.data.currentQuantity}</span>
                            </p>
                            <p className="text-xs text-zinc-600">
                                Nguồn: GET batch — dùng nhanh trên sàn bếp, không thay thế kiểm kê đầy đủ.
                            </p>
                        </div>
                    )}
                </DialogContent>
