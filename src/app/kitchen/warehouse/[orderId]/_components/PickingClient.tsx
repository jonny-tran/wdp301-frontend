...
import PickingDetailTable, {
  buildPickingLines,
  type PickingLineState,
} from "./PickingDetailTable";
...
function hasOrderPartialFulfillment(
...
const extractSuggestedBatch = (message: string) => {
  const match = message.match(/FEFO suggests Batch:\s*([\w-]+)/i);
...
export default function PickingClient({ orderId }: PickingClientProps) {
  ...
  const [suggestedBatch, setSuggestedBatch] = useState<{
    batchCode: string;
  } | null>(null);
  ...
  const completionPercent = useMemo(() => {
    ...
    const done = lines.filter(
      (l) => l.lineStatus === "verified" || l.lineStatus === "issue",
    ).length;
  ...
  const totalVerifiedUnits = useMemo(() => {
    return lines
      ...
      .reduce(
        (sum, l) =>
          sum +
          (Number.isFinite(Number(l.pickedQty)) ? Number(l.pickedQty) : 0),
        0,
      );
  }, [lines]);
  ...
  const handleChangePickedQty = (index: number, value: string) => {
    setLines((prev) =>
      prev.map((row, i) => (i === index ? { ...row, pickedQty: value } : row)),
    );
  };
  ...
  const handleConfirmLine = async (index: number) => {
    ...
    try {
      ...
      const resolvedId =
        result.batchId != null && result.batchId > 0
          ? result.batchId
          : line.batchId;
      setLines((prev) =>
        prev.map((row, i) =>
      ...
      toast.success("Đã xác nhận lô", {
        description: result.batchCode || line.batchCode,
      });
  ...
  const handleReportIssue = async (index: number, reason: string) => {
    ...
    const batchId =
      line.batchId != null && line.batchId > 0 ? line.batchId : null;
    if (batchId == null) {
      toast.error(
        "Chưa có batchId — không gửi được báo cáo. Thử xác nhận lô trước hoặc liên hệ quản trị.",
      );
      return;
    }
    ...
    try {
      ...
      setLines((prev) =>
        prev.map((row, i) =>
          i === index ? { ...row, lineStatus: "issue" as const } : row,
        ),
      );
  ...
  const runFinalizeShipment = async () => {
    ...
    if (pickedItems.length === 0) {
      toast.error(
        "Cần ít nhất một dòng đã xác nhận (xanh) với batchId và số lượng hợp lệ.",
      );
      return;
    }
    ...
    try {
      ...
      // Kiểm tra nếu là lỗi vi phạm FEFO
      if (errorMsg.includes("FEFO suggests")) {
        const batchCode = extractSuggestedBatch(errorMsg);
        if (batchCode) {
          setFinalizeDialogOpen(false);
          setSuggestedBatch({ batchCode });
          return;
        }
      }
      handleErrorApi({ error: e });
  ...
  const handleReplaceBatch = async (suggested: { batchCode: string }) => {
    setSuggestedBatch(null);

    // Chúng ta mark tất cả line về pending rồi verify lại
    setLines((prev) =>
      prev.map((line) => ({
        ...line,
        lineStatus: "pending",
        batchCode: suggested.batchCode,
      })),
    );

    toast.info(
      "Đã áp dụng mã lô FEFO đề xuất. Vui lòng xác nhận lô và xuất kho gộp lại.",
    );
  };
    ...
    <div className="space-y-6">
      ...
      >
        <AlertDialogContent className="border-2 border-zinc-200 sm:max-w-lg">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-left text-lg font-black text-zinc-950">
              ...
              <div className="space-y-3 text-left text-sm text-zinc-700">
                {finalizeShowsPartialWarning ? (
                  <p className="rounded-xl border-2 border-amber-400 bg-amber-50 p-3 font-semibold text-amber-950">
                    Cảnh báo: Bạn đang giao thiếu hàng. Theo quy tắc hệ thống,
                    phần còn thiếu sẽ bị HỦY tự động. Store sẽ phải đặt lại vào
                    ngày hôm sau. Bạn có chắc chắn muốn tiếp tục?
                  </p>
                ) : (
                  <p>
                    Bạn sắp chốt xuất kho theo các lô đã xác nhận. Hành động này
                    cập nhật shipment và tồn kho theo nghiệp vụ hiện tại.
                  </p>
                )}
              </div>
            </AlertDialogDescription>
          ...
          <AlertDialogFooter>
            ...
            >
              {finalizeBulkShipment.isPending
                ? "Đang xử lý…"
                : "Tiếp tục xuất kho"}
            </Button>
          </AlertDialogFooter>
      ...
      <AlertDialog
        open={!!suggestedBatch}
        onOpenChange={() => setSuggestedBatch(null)}
      >
        <AlertDialogContent className="border-2 border-orange-200 sm:max-w-lg">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-left text-lg font-black text-orange-700 flex items-center gap-2">
              <AlertTriangle className="h-5 w-5" />
              Vi phạm nguyên tắc FEFO
            </AlertDialogTitle>
            <AlertDialogDescription asChild>
              <div className="space-y-3 text-left text-sm text-zinc-700">
                <p>
                  Mã lô bạn vừa chọn không phải là lô có hạn sử dụng gần nhất.
                </p>
                <p className="rounded-xl border-2 border-orange-200 bg-orange-50 p-3 font-semibold text-orange-900">
                  Hệ thống đề xuất sử dụng lô:{" "}
                  <span className="font-bold text-orange-700">
                    {suggestedBatch?.batchCode}
                  </span>
                </p>
              </div>
            </AlertDialogDescription>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              className="bg-orange-600 hover:bg-orange-700 font-bold text-white text-sm"
              onClick={() =>
                suggestedBatch && handleReplaceBatch(suggestedBatch)
              }
            >
              Đồng ý thay thế bằng lô đề xuất
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <Dialog open={successDialogOpen} onOpenChange={setSuccessDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto border-2 border-emerald-200 sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-lg font-black text-emerald-900">
              Tóm tắt lô đã xuất
            </DialogTitle>
          </DialogHeader>
          {shipmentSummary ? (
                  ...
                  <TableBody>
                    {shipmentSummary.rows.map((r, i) => (
                      <TableRow key={`${r.batchCode}-${i}`}>
                        <TableCell className="max-w-[140px] text-zinc-800">
                          {r.productName}
                        </TableCell>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link
            ...
            className="rounded-xl border border-gray-200 bg-white p-2 hover:border-primary/40"
          >
            <ArrowLeftIcon className="h-5 w-5 text-text-main" />
          </Link>
          <div>
            <h1 className="text-2xl font-black text-text-main">
              Soạn hàng thủ công (FEFO)
            </h1>
            <p className="text-sm text-text-muted">
              Mã lô là thông tin chính để lấy hàng. SL soạn mặc định theo gợi ý
              — chỉnh xuống khi thiếu tồn, rồi bấm một nút xác nhận.
            </p>
          </div>
        </div>
          ...
          <section className="overflow-hidden rounded-3xl border border-zinc-200/90 bg-white shadow-sm ring-1 ring-zinc-100">
            <div className="border-b border-zinc-100 bg-gradient-to-r from-zinc-50 to-white px-4 py-3 sm:px-6">
              <h2 className="text-sm font-black uppercase tracking-wide text-zinc-700">
                Danh sách lô gợi ý (FEFO)
              </h2>
              <p className="mt-0.5 text-xs text-zinc-500">
                Mỗi lô một thẻ — không cần kéo ngang. Xác nhận = kiểm tra mã lô
                qua kho (GET).
              </p>
            </div>
              ...
              <Can
                ...
                fallback={
                  <p className="text-sm text-zinc-500">
                    Bạn không có quyền xác nhận lô.
                  </p>
                }
              >
                <PickingDetailTable
