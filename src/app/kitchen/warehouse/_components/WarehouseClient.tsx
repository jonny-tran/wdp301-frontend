...
export default function WarehouseClient({
  ...
  const parsedQuery = useMemo(
    () =>
      parseKitchenListQuery(searchParams, {
        page: 1,
        limit: 10,
        sortOrder: "DESC",
      }),
  ...
  const handlePageChange = (nextPage: number) => {
    const query = createPaginationSearchParams(searchParamsHook, {
      page: nextPage,
    });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">
            Theo dõi và xử lý các tác vụ lấy hàng tại bếp trung tâm cho các đơn
            hàng của cửa hàng.
          </p>
        </div>
        <button
