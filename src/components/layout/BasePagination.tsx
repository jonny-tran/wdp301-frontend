...
export function BasePagination({
  ...
  const start = totalItems && itemsPerPage ? (page - 1) * itemsPerPage + 1 : 0;

  const end =
    totalItems && itemsPerPage ? Math.min(page * itemsPerPage, totalItems) : 0;
      ...
      <p className="text-[11px] font-bold uppercase tracking-widest text-slate-600">
        {totalItems ? (
          <>
            ...
            trên <span className="text-slate-900">{totalItems}</span>
          </>
        ) : (
          <>
          ...
          <PaginationItem>
            <PaginationPrevious
              ...
              className={cn(
                ...
                page === 1 &&
                  "pointer-events-none font-black text-slate-900 border-gray-100",
              )}
            />
          </PaginationItem>
              ...
              {p === "..." ? (
                ...
                <PaginationLink
                  ...
                  className={cn(
                    ...
                    p === page
                      ? "bg-slate-900 border-slate-900 text-black shadow-lg shadow-slate-900/20 scale-105"
          ...
          <PaginationItem>
            <PaginationNext
              ...
              className={cn(
                "h-9 rounded-lg border border-slate-300 bg-white px-3 hover:border-primary hover:text-primary transition-all text-slate-700 font-bold text-[11px] uppercase tracking-wider",
                page === totalPages &&
                  "pointer-events-none text-slate-300 border-gray-100",
              )}
            />
          </PaginationItem>
        </PaginationContent>
