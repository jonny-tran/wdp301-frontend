...
export const claimRequest = {
    ...
    // GET /claims
    getClaims: (query: QueryClaim) => http.get<BaseResponsePagination<Claim>>(ENDPOINT_CLIENT.CLAIMS, { query }),

    // GET /claims/:id
    getClaimDetail: (id: string) => http.get<Claim>(ENDPOINT_CLIENT.CLAIM_DETAIL(id)),

    // GET /claims/my-store
    getMyStoreClaims: (query: QueryClaim) => http.get<BaseResponsePagination<Claim>>(ENDPOINT_CLIENT.CLAIM_MY_STORE, { query }),
